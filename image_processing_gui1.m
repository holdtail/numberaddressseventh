 ...
 % 旋转图像函数
   function rotateImage()
    ...
    function histData = computeHistogram(grayImage)
        histData = zeros(1, 256);% 创建一个长度为256的零数组
        for i = 1:numel(grayImage) % 遍历图像中的每个像素
            histData(grayImage(i) + 1) = histData(grayImage(i) + 1) + 1;% 统计灰度值的频率
        end
    end
    ...
    function eqImage = histogramEqualization(grayImage)
        % 计算累积分布函数 (CDF)
        histData = computeHistogram(grayImage); % 计算灰度直方图
        cdf = cumsum(histData) / numel(grayImage);% 计算累积分布函数（CDF），并归一化
        % 归一化映射
        eqImage = uint8(255 * cdf(double(grayImage) + 1)); % 使用CDF对图像进行均衡化
    end
...
function enhanceContrast()
    ...
    % 第二个子图：线性对比度增强
    linearEnhanced = imadjust(grayImg, [0.2, 0.8], [0, 1]);%imadjust函数对图像的灰度值进行线性拉伸或压缩
    subplot(2, 2, 2);
...
% 椒盐噪声添加函数
    ...
    if ~isempty(img)
        p = noiseMeanEdit.Value;  % 获取用户输入的噪声均值（0到1之间）
        noisyImg = img;
        ...
        imshow(noisyImg, 'Parent', ax2);
        processedImg = noisyImg;
        disp(['添加椒盐噪声，噪声强度: ', num2str(p)]);
    end
end

% 平滑操作
    function applySmoothingOperation(operation)
    if strcmp(operation, '均值平滑') && ~isempty(processedImg)
        % 步骤 1: 直接应用均值平滑
        processedImg1 = mean_smooth_custom(processedImg, 3);  % 使用3x3均值滤波器

        % 显示处理后的图像
        imshow(processedImg1, 'Parent', ax2);

        % 计算并显示熵值
        entropyValue = entropy(rgb2gray(processedImg1));  % 使用灰度图计算熵值
...
function smoothedImg = mean_smooth_custom(inputImg, kernelSize)
 ...
 % 处理频域变换函数
    function applyFrequencyDomainOperation(option)
        ...
        switch option
            case '傅里叶变换'
                processedImg1 = applyFourierTransform(processedImg);
            case '离散余弦变换'
                processedImg1 = applyDCT(processedImg);
            otherwise
                processedImg1 = processedImg; % 无操作
        end
        % 显示处理后的图像
        imshow(processedImg1, 'Parent', ax2);
    end
  ...
  % 锐化操作选择回调函数
   ...
   % 频域操作的回调函数
    function applyFilteringDomainOperation(opType)
        ...
        % 转换为灰度图像
        grayImg = rgb2grayCustom(processedImg);
        ...
        switch opType
            case '低通滤波'
                processedImg1 = lowPassFilter(fshift);
            case '高通滤波'
                processedImg1 = highPassFilter(fshift);
            case '带通滤波'
                processedImg1 = bandPassFilter(fshift);
            case '带阻滤波'
                processedImg1 = bandStopFilter(fshift);
            otherwise
                processedImg1 = grayImg;
        end

        % 将频域处理后的图像转换回空间域
        processedImg1 = real(ifft2(ifftshift(processedImg1)));
        
        % 显示处理后的图像
        imshow(uint8(processedImg1), 'Parent', ax2);
    end
